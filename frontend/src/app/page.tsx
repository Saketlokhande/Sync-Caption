...
import { Type, Download, Wand2, CheckCircle2, AlertCircle, ChevronDown } from 'lucide-react';
...
export default function Home() {
  ...
  const [style, setStyle] = useState<'standard' | 'news' | 'karaoke'>('standard');
  ...
  const handleRender = async () => {
    ...
    setError('');
    try {
      const backendVideoUrl = `http://localhost:8000/uploads/${filename}`;
      const result = await renderVideo(backendVideoUrl, captions, style, videoDuration, videoDimensions);
      setDownloadUrl(`http://localhost:8000${result.url}`);
    ...
    <main className="min-h-screen bg-background text-foreground relative overflow-x-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
        <Jellyfish
          ...
          tentacles={[
            { left: '50%', height: '200px', delay: '0s', animation: 'animate-tentacle-1', color: 'rgba(34, 211, 238, 0.3)' },
            { left: '45%', height: '180px', delay: '0.5s', animation: 'animate-tentacle-2', color: 'rgba(34, 211, 238, 0.25)' },
            { left: '55%', height: '190px', delay: '1s', animation: 'animate-tentacle-1', color: 'rgba(34, 211, 238, 0.25)' },
          ]}
        />
        ...
        <Jellyfish
          ...
          tentacles={[
            { left: '50%', height: '170px', delay: '0.3s', animation: 'animate-tentacle-2', color: 'rgba(192, 132, 252, 0.3)' },
            { left: '48%', height: '160px', delay: '0.8s', animation: 'animate-tentacle-1', color: 'rgba(244, 114, 182, 0.25)' },
            { left: '52%', height: '165px', delay: '1.3s', animation: 'animate-tentacle-2', color: 'rgba(192, 132, 252, 0.25)' },
          ]}
        />
        ...
        <Jellyfish
          ...
          tentacles={[
            { left: '50%', height: '150px', delay: '0.2s', animation: 'animate-tentacle-1', color: 'rgba(52, 211, 153, 0.3)' },
            { left: '47%', height: '140px', delay: '0.7s', animation: 'animate-tentacle-2', color: 'rgba(34, 197, 94, 0.25)' },
          ]}
        />
        ...
        <Jellyfish
          ...
          animationDelay="2s"
          tentacles={[
            { left: '50%', height: '120px', delay: '0.4s', animation: 'animate-tentacle-2', color: 'rgba(59, 130, 246, 0.3)' },
            { left: '48%', height: '110px', delay: '0.9s', animation: 'animate-tentacle-1', color: 'rgba(34, 211, 238, 0.25)' },
          ]}
        />
      ...
      <section className="h-screen flex flex-col items-center justify-center px-4 relative">
        ...
        <div className="max-w-3xl mx-auto text-center space-y-5 relative z-10">
          ...
          <p className="text-lg md:text-xl text-muted-foreground/90 max-w-xl mx-auto leading-relaxed">
            Transform your videos with professional, AI-generated captions in seconds. 
            Upload, customize, and export with ease.
          </p>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 space-y-4">
              ...
              {/* Step 2: Transcribe */}
              <Card className={`border-white/10 bg-white/5 backdrop-blur-xl transition-opacity duration-300 ${!filename ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-base">
                ...
                <CardContent>
                  ...
                  >
                    ...
                    {loading && status === 'Generating captions...' ? 'Transcribing...' : 'Auto-Generate Captions'}
                  </Button>
                </CardContent>
              </Card>

              {/* Step 3: Style */}
              <Card className={`border-white/10 bg-white/5 backdrop-blur-xl transition-opacity duration-300 ${captions.length === 0 ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <div className="flex items-center justify-center w-6 h-6 rounded-full bg-pink-500/20 text-pink-400 text-xs font-bold">3</div>
                    Choose Style
                  </CardTitle>
                </CardHeader>
                <CardContent>
                    ...
                    {['standard', 'news', 'karaoke'].map((s) => (
                      ...
                      >
                        <div className="relative z-10 flex flex-col items-center gap-1">
                          ...
                          {style === s && <CheckCircle2 className="w-4 h-4 text-primary" />}
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              <Card className={`border-white/10 bg-white/5 backdrop-blur-xl transition-opacity duration-300 ${captions.length === 0 ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-base">
                    <div className="flex items-center justify-center w-6 h-6 rounded-full bg-green-500/20 text-green-400 text-xs font-bold">4</div>
                    Export
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  >
                    ...
                    {loading && status === 'Rendering video...' ? 'Rendering...' : 'Render & Download'}
                  </Button>
                  
                  {downloadUrl && (
                    <div className="p-3 bg-green-500/10 border border-green-500/20 rounded-lg backdrop-blur-md">
                      <p className="text-center text-green-400 mb-2 font-medium text-sm">Video ready!</p>
                      <a
                        href={downloadUrl}
                        target="_blank"
              ...
              <div className="sticky top-8">
                <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden shadow-2xl ring-1 ring-white/10 backdrop-blur-xl">
                  ...
                  <div className="p-1 bg-black">
                    {videoUrl ? (
                      <RemotionPlayer videoUrl={videoUrl} captions={captions} style={style} />
                    ) : (
